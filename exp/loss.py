...
class SquaredLoss(Loss):
    ...
    def forward(self, X, w, y):
        ...
        # calculate loss
        losses = .5 * (y- np.einsum('j,kj -> k', w, X)) ** 2
    ...
    def backward(self, X, w, y):
        ...
        partials = np.einsum('k,kj -> kj', (y- np.einsum('j,kj -> k', w, X)), X)

        # average
        gradient = -np.sum(partials, axis=0) / N
...
class HingeLoss(Loss):
    ...
    def forward(self, X, w, y):
        ...
        # calculate losses
        losses = np.maximum(np.zeros(N), np.ones(N) - y * np.einsum('j,kj -> k', w, X))
    ...
    def backward(self, X, w, y):
        ...
        # calculate partial gradients
        condition = y * np.einsum('j,kj -> k', w, X)
        bad = np.where(condition >= 1)
        partials = np.einsum('k,kj -> kj', -y, X)
        partials[bad] = 0
